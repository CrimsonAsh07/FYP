...
def main(config):
    ...
    data_dir = "RE_ID_VAE/data/bounding_box_test"
    ...
    for i in range(num):
        image_id = int(data_file_names[i].split("_")[0])
        distances = []
        idjs = []
        for j in range(num):
            if(i == j):
                continue
            image_idj = int(data_file_names[j].split("_")[0])
  ...
  # Find indices of k largest elements in the first values array
        ...
        rankValTuple = []
        for rank in ranks:
            rankVal = count_rank_k(sorted_dist, rank, image_id)
    ...
    # print(f"Rank-1 Accuracy: {meanKs[0]*100 :.4f}")
    # print(f"Rank-5 Accuracy: {meanKs[1]*100:.4f}")
    # print(f"Rank-10 Accuracy: {meanKs[2]*100:.4f}")
